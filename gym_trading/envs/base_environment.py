...
class BaseEnvironment(Env, ABC):
    ...
    def __init__(self,
                 symbol: str,
                 #TODO - Remove fitting and testing files once complete
                 fitting_file: str,
                 testing_file: str,
                 data_dir:str,
                 feature_set:list,
        ...
        self.done = False
        self.target_file_repeats = target_file_repeats
        ...
        self.data_pipeline = DataPipeline(
            ...
            include_buys_sells = self.include_buys_sells,
        )
        
        if kwargs['use_demo']==False:
            #intilialize zscore scalers for all the target data
            self.data_pipeline.initialize_zscalers_ema(self.data_dir)

            #load data
            self._midpoint_prices, self._raw_data, self._normalized_data = \
                self.data_pipeline.load_environment_data_window(
                    data_dir=self.data_dir)

            print(self._normalized_data.columns.to_list())
        else:
    ...
    def reset(self) -> np.ndarray:
        ...
        #if we are onle using multiple target files
        #load in new training day data every n episodes
        if self.kwargs['use_demo']==False and self.episode_stats.number_of_episodes % self.target_file_repeats == 0:
            #load data
            self._midpoint_prices, self._raw_data, self._normalized_data = \
                self.data_pipeline.load_environment_data_window(
                    data_dir=self.data_dir, 
        ...
        if self.broker.total_trade_count > 0 or self.broker.realized_pnl != 0.:
            ...
            print('PnL Ratio: {:.4f}%'.format(
                self.broker.profitable_trade_ratio * 100.))
    ...
    def _get_step_observation(self, step_action: int = 0) -> np.ndarray:
        ...
        step_indicator_features = self._create_indicator_features() if "I" in self.feature_set else []
